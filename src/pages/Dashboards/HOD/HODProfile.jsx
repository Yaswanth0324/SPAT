...
import { User, Mail, Phone, Lock, Upload, KeyRound, ShieldAlert, Award, FileText, CheckCircle2 } from 'lucide-react';
import { useAuth } from '../../../context/AuthContext';
import { updateUser, getUsers } from '../../../utils/localStorage';
import { useToast, Avatar, Badge } from '../../../components/ui/UIComponents';
...
export const HODProfile = () => {
  ...
  const handleSuccessionRequest = (e) => {
    e.preventDefault();
    if (!newHodName.trim() || !newHodEmail.trim() || !newHodPassword.trim()) {
      showToast('Please fill in Name, Email, and Password! ⚠️', 'error');
      return;
    }

    // Check if new HOD email is already in use
    const emailExists = getUsers().some(u => u.email.toLowerCase() === newHodEmail.trim().toLowerCase());
    if (emailExists) {
    ...
    const request = {
      ...
      email: newHodEmail.trim().toLowerCase(),
      password: newHodPassword,
      phone: newHodPhone.trim(),
      status: 'pending',
    ...
    showToast('HOD Succession Request submitted to College Admin! ✓', 'success');
  };
    ...
    <div className="animate-fade-in space-y-8 pb-12">
      ...
      <div className="border-b border-slate-100 dark:border-dark-800 pb-5">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Manage your personal information, department leadership status, and succession parameters.
        </p>
      </div>
          ...
          {activeSuccession && (
            <div className="p-4 bg-orange-500/5 border border-orange-500/20 rounded-2xl">
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed mb-3">
                Replacing leadership with **{user.successionRequest.name}** ({user.successionRequest.email}). This change will take place upon College Admin approval.
              </p>
              <button
                type="button"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card p-6 md:p-8 border border-slate-100 dark:border-dark-800">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <User className="w-5 h-5 text-orange-500" /> HOD Profile Info
              </h3>
              {!isEditingInfo && (
                <button
                  type="button"
          ...
          <div className="card p-6 md:p-8 border border-slate-100 dark:border-dark-800 relative overflow-hidden bg-gradient-to-br from-white to-slate-50/50 dark:from-dark-900 dark:to-dark-950/20">
            ...
            <h3 className="font-display text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2 mb-2">
              ...
              HOD Handover & succession Request
            </h3>
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6 leading-relaxed">
              Transition leadership to a new HOD without losing any existing department data. Input the credentials of the new HOD. Upon College Admin approval, the handover takes place in-place.
            </p>
            ...
            <form onSubmit={handleSuccessionRequest} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5">
                    New HOD Contact Phone (Optional)
                  </label>
                  <input
                    type="text"
                    value={newHodPhone}
